...
void NB::Universe::step(double dt) {
    ...
    for (size_t i = 0; i < bodies.size(); i++) {
        std::cerr << "Body " << i << " Initial Position: (" << bodies[i]->position().x << ", " << bodies[i]->position().y << ")\n";
        std::cerr << "Body " << i << " Initial Velocity: (" << bodies[i]->velocity().x << ", " << bodies[i]->velocity().y << ")\n";

        if (bodies[i]->mass() == 0) {
            newPositions[i] = bodies[i]->position();
            newVelocities[i] = bodies[i]->velocity();
            std::cerr << "Skipping massless body " << i << "\n";
            continue;  
        }
    ...
    for (size_t i = 0; i < bodies.size(); i++) {
        ...
        std::cerr << "Body " << i << " New Position: (" << bodies[i]->position().x << ", " << bodies[i]->position().y << ")\n";
        std::cerr << "Body " << i << " New Velocity: (" << bodies[i]->velocity().x << ", " << bodies[i]->velocity().y << ")\n";
    }
}
