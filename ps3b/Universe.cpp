...
std::istream& operator>>(std::istream& in, Universe& universe) {
    ...
    double radius;
    in >> n >> radius;
    universe.universeRadius = radius;  // ✅ Corrected name
...
void Universe::draw(sf::RenderTarget& window, sf::RenderStates states) const {
    window.draw(*backgroundSprite);  // ✅ Fixed variable name

    for (const auto& body : bodies) {
        sf::Vector2f pos = body->position();
        float screenX = (pos.x / universeRadius) * 400 + 400;  // ✅ Fixed variable
        float screenY = (pos.y / universeRadius) * 400 + 400;  // ✅ Fixed variable
