...
ptime parseTimestampFromStr(const string& ts) {
    static const regex iso_pattern(R"((\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}))");
    static const regex alt_pattern(R"((\w{3}\s+\d{1,2}\s\d{2}:\d{2}:\d{2}))");

    smatch match;
    if (regex_match(ts, match, iso_pattern)) {
        return time_from_string(ts);
    }

    if (regex_match(ts, match, alt_pattern)) {
        string fullTimestamp = "2013 " + ts;
...
int main(int argc, char* argv[]) {
    ...
    const regex start_pattern(R"((\w{3}\s+\d{1,2}\s\d{2}:\d{2}:\d{2}).*\(log\.c\.166\) server started)");
    const regex end_pattern(R"((\w{3}\s+\d{1,2}\s\d{2}:\d{2}:\d{2}).*oejs\.AbstractConnector:Started SelectChannelConnector)");
    ...
    while (getline(infile, line)) {
        ...
        if (regex_search(line, match, start_pattern)) {
            string ts = match.str(1);
            ptime timestamp = parseTimestampFromStr(ts);
            if (!timestamp.is_not_a_date_time()) {
                events.push_back({lineNum, ts, timestamp, true});
            }
        } else if (regex_search(line, match, end_pattern)) {
            string ts = match.str(1);
            ptime timestamp = parseTimestampFromStr(ts);
            if (!timestamp.is_not_a_date_time()) {
                events.push_back({lineNum, ts, timestamp, false});
            }
        }
    }
